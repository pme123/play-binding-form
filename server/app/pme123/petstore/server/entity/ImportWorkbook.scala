...
import pme123.form.shared.services.{Logging, SPAException, User}
...
case class ImportWorkbook(wb: Workbook)
  ...
  lazy val categories: Try[Seq[Try[FormCategory]]] =
    getSheet(sheetCategories) map { s =>
      rows(s).map { row =>
        Try(
          FormCategory(cellAsString(row.getCell(col0)),
  ...
  lazy val formProducts: Try[Seq[Try[FormProduct]]] =
    getSheet(sheetFormProducts) map { s =>
      rows(s).map { row =>
        Try(
          FormProduct(cellAsString(row.getCell(col0)),
            cellAsString(row.getCell(col1)),
            findCategory(cellAsString(row.getCell(col2))).get,
            cellAsStringSet(row.getCell(col3)))
        ) recoverWith failure(sheetFormProducts, row.getRowNum)
      }
    }

  lazy val forms: Try[Seq[Try[Form]]] = {
    getSheet(sheetForms) map { s =>
      rows(s).map { row =>
        Try(
          Form(cellAsString(row.getCell(col0)),
            cellAsString(row.getCell(col1)),
            cellAsDouble(row.getCell(col2)),
            findProduct(cellAsString(row.getCell(col3))).get,
            FormStatus.withNameInsensitive(cellAsString(row.getCell(col4))),
  ...
  private def findCategory(name: String): Try[FormCategory] = findContent(name, categories)

  private def findProduct(name: String): Try[FormProduct] = findContent(name, formProducts)
...
object ImportWorkbook {
  ...
  val sheetFormProducts = "products"
